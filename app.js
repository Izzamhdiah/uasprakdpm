...
const mongoUrl = "mongodb+srv://izzamahdiaheriani:admin22@cluster0.xva336l.mongodb.net/?retryWrites=true&w=majority";
...
app.post("/createschedule", async (req, res) => {
      const { tgl, hari, genre, jam, judul } = req.body;
...
app.post("/updateschedule", async (req, res) => {
      const { scheduleId,  tgl, hari, genre, jam, judul } = req.body;

      try {
            const updatedSchedule = await mongoose.model("Schedules").findOneAndUpdate(
                  { _id: scheduleId },
                  { $set: {  tgl, hari, genre, jam, judul } },
                  { new: true }
            );
